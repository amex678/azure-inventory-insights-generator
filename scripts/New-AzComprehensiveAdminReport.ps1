...
param(
    ...
    # 任意: Copilot Cloud Agent が生成した散文（本文）JSON。指定があり有効なスロットは
    # ルールベース散文の代わりに採用される。数値・表・スコアは常に本スクリプトが確定する。
    [string]$BodyJson      = '',
...
# ==========================================================================
# LLM 散文の安全レンダリング
#   Copilot Agent が書いた本文は「プレーンテキスト + 限定 Markdown 強調」のみ許可。
#   HTML は書かせない。ここで HTML escape → 限定強調(**bold** / `code` / *italic*)
#   のみ復元することで XSS / タグ注入を根本封じする（CI sanitize の二重防御）。
# ==========================================================================
function ConvertTo-SafeProse {
    param([string]$s)
...
# body.json の読み込み（存在し妥当な場合のみ narrative source を copilot に）
$body = $null
$narrativeSource = 'fallback'
$narrativeReason = 'body.json が指定されていません（ルールベース散文で生成）'
if (-not [string]::IsNullOrWhiteSpace($BodyJson)) {
    if (Test-Path $BodyJson) {
        try {
            $raw = Get-Content -Path $BodyJson -Raw -Encoding UTF8
            $body = $raw | ConvertFrom-Json
            if ($body -and $body.slots) {
                $narrativeSource = 'copilot'
                $narrativeReason = 'Copilot Cloud Agent が生成した本文を採用'
            } else {
                $narrativeReason = 'body.json に slots がありません（フォールバック）'
            }
        } catch {
            $narrativeReason = "body.json の解析に失敗（フォールバック）: $($_.Exception.Message)"
        }
    } else {
        $narrativeReason = "body.json が見つかりません: $BodyJson（フォールバック）"
    }
}

# スロット取得: body に存在すれば LLM 散文、なければ $null（＝呼び出し側でフォールバック）
function Get-Slot {
    param([string]$name)
    if (-not $body -or -not $body.slots) { return $null }
    $val = $body.slots.$name
    if ($null -eq $val) { return $null }
    return $val
}

# assessment スロット {summary, findings[], recommendation} を既存レイアウトの HTML 断片に変換
function Format-AssessmentSlot {
    param($slot)
    if ($null -eq $slot) { return $null }
    $sb = [System.Text.StringBuilder]::new()
    if ($slot.summary) { [void]$sb.Append("<p>$(ConvertTo-SafeProse $slot.summary)</p>`n") }
    $findings = @($slot.findings)
    if ($findings.Count -gt 0) {
        [void]$sb.Append("<p><strong>所見:</strong></p>`n<ul style=""line-height:1.8;"">")
        foreach ($f in $findings) { [void]$sb.Append("<li>$(ConvertTo-SafeProse ([string]$f))</li>") }
        [void]$sb.Append("</ul>`n")
    }
    if ($slot.recommendation) { [void]$sb.Append("<p><strong>推奨アクション:</strong> $(ConvertTo-SafeProse $slot.recommendation)</p>") }
    $out = $sb.ToString()
...
# キーメトリクス（KPI カード）
$kpiCards = @(
    @{ Label='総合スコア'; Value="$overallScore"; Unit='/100'; Color=$overallVerdict.Color; Sub=$overallVerdict.Text }
    @{ Label='高リスク NSG'; Value="$nRisky"; Unit='件'; Color=$(if ($nRisky -gt 0){'var(--high)'}else{'var(--ok)'}); Sub='22/3389 公開' }
    @{ Label='Owner 付与'; Value="$aOwner"; Unit='件'; Color=$(if ($aOwnerMg -gt 0){'var(--high)'}elseif ($aOwner -gt 2){'var(--med)'}else{'var(--ok)'}); Sub="MG スコープ $aOwnerMg" }
    @{ Label='Secure Score'; Value="$secureScorePct"; Unit='%'; Color=$(if($secureScorePct -ge 80){'var(--ok)'}elseif($secureScorePct -ge 50){'var(--med)'}else{'var(--high)'}); Sub="Healthy $dHealthy / $dEvaluated" }
    @{ Label='Advisor High'; Value="$advHigh"; Unit='件'; Color=$(if ($advHigh -ge 10){'var(--high)'}elseif ($advHigh -gt 0){'var(--med)'}else{'var(--ok)'}); Sub="総 $advTotal 件" }
    @{ Label='タグ付与率'; Value="$tagCoverage"; Unit='%'; Color=$(if ($tagCoverage -ge 80){'var(--ok)'}elseif ($tagCoverage -ge 50){'var(--med)'}else{'var(--high)'}); Sub="未付与 $untaggedCount" }
)
...
# --- LLM 散文オーバーライド（exec サマリ）---
$verdictDescDefault = "サブスクリプション <code>$(ConvertTo-HtmlEncoded $subName)</code> は、リソース <strong>$rTotal 件 / $rRGs RG / $rLocs リージョン</strong> 構成。NSG / RBAC / Defender / Advisor / ガバナンス の 5 軸を加重平均した総合スコアです。"
$verdictDescText = $verdictDescDefault
$slotVerdict = Get-Slot 'verdictDesc'
if ($slotVerdict) { $verdictDescText = ConvertTo-SafeProse ([string]$slotVerdict) }

$slotConcerns = Get-Slot 'concerns'
if ($slotConcerns) {
    $items = @($slotConcerns | Where-Object { -not [string]::IsNullOrWhiteSpace([string]$_) })
    if ($items.Count -gt 0) { $concernsHtml = ($items | ForEach-Object { "<li>$(ConvertTo-SafeProse ([string]$_))</li>" }) -join "`n" }
}
$slotStrengths = Get-Slot 'strengths'
if ($slotStrengths) {
    $items = @($slotStrengths | Where-Object { -not [string]::IsNullOrWhiteSpace([string]$_) })
    if ($items.Count -gt 0) { $strengthsHtml = ($items | ForEach-Object { "<li>$(ConvertTo-SafeProse ([string]$_))</li>" }) -join "`n" }
}
$slotFocus = Get-Slot 'focus'
if ($slotFocus) {
    $items = @($slotFocus | Where-Object { -not [string]::IsNullOrWhiteSpace([string]$_) })
    if ($items.Count -gt 0) { $focusHtml = ($items | ForEach-Object { "<li>$(ConvertTo-SafeProse ([string]$_))</li>" }) -join "`n" }
}
...
<div class="verdict-bar" style="border-left:5px solid $($overallVerdict.Color);">
  ...
  <div>
    ...
    <div class="verdict-desc">$verdictDescText</div>
  </div>
</div>
...
# --- LLM 散文オーバーライド（リソース / RBAC / NSG アセスメント）---
$slotRes = Format-AssessmentSlot (Get-Slot 'resourcesAssessment')
if ($slotRes) { $resourcesAssessment = $slotRes }
$slotRbac = Format-AssessmentSlot (Get-Slot 'rbacAssessment')
if ($slotRbac) { $rbacAssessment = $slotRbac }
$slotNsg = Format-AssessmentSlot (Get-Slot 'nsgAssessment')
if ($slotNsg) { $nsgAssessment = $slotNsg }
...
# --- LLM 散文オーバーライド（Top 5 リスク: fact/reason/recommend のみ。Links は PS 固定）---
$slotRisks = Get-Slot 'risks'
if ($slotRisks) {
    foreach ($r in $risks) {
        $ov = $slotRisks | Where-Object { [int]$_.rank -eq [int]$r.Rank } | Select-Object -First 1
        if ($ov) {
            if ($ov.fact)      { $r.Fact      = ConvertTo-SafeProse ([string]$ov.fact) }
            if ($ov.reason)    { $r.Reason    = ConvertTo-SafeProse ([string]$ov.reason) }
            if ($ov.recommend) { $r.Recommend = ConvertTo-SafeProse ([string]$ov.recommend) }
        }
    }
}
...
# --- LLM 散文オーバーライド（アクションプラン: detail を window で突合）---
$slotPlan = Get-Slot 'actionPlan'
foreach ($p in $plan) {
    if ($slotPlan) {
        $ov = $slotPlan | Where-Object { [string]$_.window -eq [string]$p.Window } | Select-Object -First 1
        if ($ov -and $ov.detail) { $p.Detail = [string]$ov.detail }
        if ($ov -and $ov.title)  { $p.Title  = [string]$ov.title }
    }
    # fallback / override いずれも安全変換（以降 Detail は安全 HTML として扱う）
    $p.Detail = ConvertTo-SafeProse ([string]$p.Detail)
}
$planRows = ($plan | ForEach-Object {
    "<tr><td class='nowrap'>$(ConvertTo-HtmlEncoded $_.Window)</td><td>$(ConvertTo-HtmlEncoded $_.Owner)</td><td><strong>$(ConvertTo-HtmlEncoded $_.Title)</strong><br><span class='muted'>$($_.Detail)</span></td></tr>"
}) -join "`n"
...
# --- LLM 散文オーバーライド（Defender / Advisor アセスメント）---
$slotDef = Format-AssessmentSlot (Get-Slot 'defenderAssessment')
if ($slotDef) { $defenderAssessment = $slotDef }
$slotAdv = Format-AssessmentSlot (Get-Slot 'advisorAssessment')
if ($slotAdv) { $advisorAssessment = $slotAdv }

# --- ナラティブ出所バナー（AI 生成 or フォールバックを運用可視化）---
$runIdText = if ([string]::IsNullOrWhiteSpace($RunId)) { 'n/a' } else { $RunId }
$modelText = if ($body -and $body.generator -and $body.generator.model) { [string]$body.generator.model } else { 'n/a' }
if ($narrativeSource -eq 'copilot') {
    $bannerBg = 'linear-gradient(90deg,#7c3aed,#0ea5e9)'
    $bannerText = "&#129504; 本文は GitHub Copilot Cloud Agent が生成（数値・スコアは PowerShell が確定） ／ model: $(ConvertTo-HtmlEncoded $modelText) ／ run_id: $(ConvertTo-HtmlEncoded $runIdText)"
} else {
    $bannerBg = 'linear-gradient(90deg,#475569,#64748b)'
    $bannerText = "&#9881;&#65039; 本文はルールベース（フォールバック）で生成 ／ 理由: $(ConvertTo-HtmlEncoded $narrativeReason) ／ run_id: $(ConvertTo-HtmlEncoded $runIdText)"
}
$narrativeBanner = "<div style=""background:$bannerBg; color:#fff; font-size:12px; padding:8px 16px; border-radius:8px; margin-bottom:14px;"">$bannerText</div>"
...
<footer>Generated by Azure PowerShell (Resources / RBAC / NSG / Defender for Cloud / Advisor) ／ Narrative: $narrativeSource ／ run_id: $runIdText</footer>
<script>
(function () {
