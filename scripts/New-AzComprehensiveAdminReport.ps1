...
$kpiCardsHtml = ($kpiCards | ForEach-Object {
    "<div class='kpi'><div class='kpi-label'>$(Encode-Html $_.Label)</div><div class='kpi-value' style='color:$($_.Color)'>$(Encode-Html $_.Value)<span class='kpi-unit'>$(Encode-Html $_.Unit)</span></div><div class='kpi-sub'>$(Encode-Html $_.Sub)</div></div>"
}) -join "`n"
...
<div class="verdict-bar" style="border-left:5px solid $($overallVerdict.Color);">
  ...
  <div>
    ...
    <div class="verdict-desc">サブスクリプション <code>$(Encode-Html $subName)</code> は、リソース <strong>$rTotal 件 / $rRGs RG / $rLocs リージョン</strong> 構成。NSG / RBAC / Defender / Advisor / ガバナンス の 5 軸を加重平均した総合スコアです。</div>
  </div>
</div>
...
$rTopTypeText = if ($rTopType) { "$(Encode-Html $rTopType.Name)（$($rTopType.Count) 件）" } else { 'なし' }
...
$nNsgIssues = @()
if ($nRisky -gt 0) {
    $riskySample = ($riskyList | Select-Object -First 3 | ForEach-Object { "<code>$(Encode-Html $_.NsgName)/$(Encode-Html $_.RuleName)</code>" }) -join ' / '
...
$summaryRowsHtml = ($summaryRows | ForEach-Object {
    "<tr><td><span class='dom dom-$($_.Domain.ToLower())'>$(Encode-Html $_.Domain)</span></td><td>$(Encode-Html $_.Item)</td><td class='num'>$(Encode-Html ([string]$_.Value))</td></tr>"
}) -join "`n"

# === 3. 潜在リスク Top 5（横断） ===
# 各リスクの観察事実は実データから引用
$riskyNsgFact = if ($nRisky -gt 0) {
    $sample = $riskyList | Select-Object -First 3 | ForEach-Object { "<code>$(Encode-Html $_.NsgName)/$(Encode-Html $_.RuleName)</code>" }
...
$riskHtml = ($risks | ForEach-Object {
    ...
    $linkHtml = ($_.Links | ForEach-Object {
        "<li><a href='$($_.Url)' target='_blank' rel='noopener'>$(Encode-Html $_.Text)</a></li>"
    }) -join ''
    @"
...
$planRows = ($plan | ForEach-Object {
    "<tr><td class='nowrap'>$(Encode-Html $_.Window)</td><td>$(Encode-Html $_.Owner)</td><td><strong>$(Encode-Html $_.Title)</strong><br><span class='muted'>$(Encode-Html $_.Detail)</span></td></tr>"
}) -join "`n"
...
$nsgAppendix = ($nsgRules | Sort-Object @{Expression='IsRiskyMgmtFromInternet';Descending=$true}, NsgName, Priority | Select-Object -First 30 | ForEach-Object {
    ...
    "<tr$rowClass>" +
    "<td>$(Encode-Html $_.NsgName)</td>" +
    "<td class='num'>$(Encode-Html ([string]$_.Priority))</td>" +
    "<td>$(Encode-Html $_.RuleName)</td>" +
    "<td>$(Encode-Html $_.Direction)</td>" +
    "<td>$(Encode-Html $_.Access)</td>" +
    "<td>$(Encode-Html $_.SourceAddressPrefix)</td>" +
    "<td>$(Encode-Html $_.DestinationPortRange)</td>" +
    "</tr>"
}) -join "`n"
...
$defenderAppendix = ($defender | Where-Object Status -eq 'Unhealthy' | Sort-Object @{Expression='Severity';Descending=$true}, DisplayName | Select-Object -First 30 | ForEach-Object {
    ...
    "<tr>" +
    "<td><span class='sev-badge $sevClass'>$(Encode-Html $sev)</span></td>" +
    "<td>$(Encode-Html $_.DisplayName)</td>" +
    "<td>$(Encode-Html $_.ResourceType)</td>" +
    "<td>$(Encode-Html $_.ResourceName)</td>" +
    "<td>$(Encode-Html $_.ResourceGroupName)</td>" +
    "</tr>"
}) -join "`n"
...
$advisorAppendix = ($advisor | Sort-Object @{Expression='Impact';Descending=$true}, Category | Select-Object -First 30 | ForEach-Object {
    ...
    "<tr>" +
    "<td>$(Encode-Html $_.Category)</td>" +
    "<td><span class='sev-badge $impClass'>$(Encode-Html $imp)</span></td>" +
    "<td>$(Encode-Html $_.Problem)</td>" +
    "<td>$(Encode-Html $_.ResourceType)</td>" +
    "<td>$(Encode-Html $_.ResourceName)</td>" +
    "</tr>"
}) -join "`n"
...
$rTypeTopRows = ($resources | Group-Object ResourceType | Sort-Object Count -Descending | Select-Object -First 15 | ForEach-Object {
    "<tr><td><code>$(Encode-Html $_.Name)</code></td><td class='num'>$($_.Count)</td></tr>"
}) -join "`n"
...
$rRgTopRows = ($resources | Group-Object ResourceGroupName | Sort-Object Count -Descending | Select-Object -First 15 | ForEach-Object {
    "<tr><td>$(Encode-Html $_.Name)</td><td class='num'>$($_.Count)</td></tr>"
}) -join "`n"
...
$rUntaggedRows = ($resources | Where-Object { [string]::IsNullOrWhiteSpace($_.Tags) } | Sort-Object ResourceGroupName, Name | Select-Object -First 30 | ForEach-Object {
    "<tr><td>$(Encode-Html $_.Name)</td><td>$(Encode-Html $_.ResourceGroupName)</td><td><code>$(Encode-Html $_.ResourceType)</code></td><td>$(Encode-Html $_.Location)</td></tr>"
}) -join "`n"
...
$aOwnerRows = ($assignments | Where-Object RoleDefinitionName -in 'Owner','User Access Administrator' | Sort-Object RoleDefinitionName, ScopeKind, DisplayName | Select-Object -First 30 | ForEach-Object {
    ...
    "<tr><td><span class='sev-badge $roleClass'>$(Encode-Html $_.RoleDefinitionName)</span></td><td>$(Encode-Html $_.DisplayName)</td><td>$(Encode-Html $_.ObjectType)</td><td>$(Encode-Html $_.ScopeKind)</td></tr>"
}) -join "`n"
...
$aOrphanRows = ($assignments | Where-Object { [string]::IsNullOrWhiteSpace($_.DisplayName) } | Sort-Object RoleDefinitionName | Select-Object -First 30 | ForEach-Object {
    "<tr><td>$(Encode-Html $_.RoleDefinitionName)</td><td><code>$(Encode-Html $_.ObjectId)</code></td><td>$(Encode-Html $_.ObjectType)</td><td>$(Encode-Html $_.ScopeKind)</td></tr>"
}) -join "`n"
...
if ($defenderEnabled) {
    ...
    $dTopTypeHtml = ($dTopType | ForEach-Object {
        ...
        "<tr><td>$(Encode-Html $typeName)</td><td class='num'>$($_.Count)</td></tr>"
    }) -join "`n"

    # High 件の詳細
    $dHighHtml = if ($dHigh -gt 0) {
        ($dHighItems | ForEach-Object {
            "<tr><td><span class='sev-badge sev-high'>High</span></td><td>$(Encode-Html $_.Name)</td><td class='num'>$($_.Count)</td></tr>"
        }) -join "`n"
    } else {
    ...
    foreach ($cat in @('Cost','OperationalExcellence','Performance','HighAvailability','Security')) {
        ...
        $rows = if ($items -and $items.Count -gt 0) {
            ($items | ForEach-Object {
                "<li><strong>$(Encode-Html $_.Name)</strong> <span class='muted'>($($_.Count) 件)</span></li>"
            }) -join ''
        } else {
...
<header class="top">
  ...
  <div class="sub">サブスクリプション: $(Encode-Html $subName) ($subId) ／ 生成日時: $generated ／ 入力: resources ($rTotal) · rbac ($aTotal) · nsg ($nRules) · defender ($dTotal) · advisor ($advTotal)</div>
</header>
<main>
...
Write-Host "Comprehensive Admin HTML: $OutputPath" -ForegroundColor Green
