...
| Microsoft 365 Message Center | 公開許可したタイトル / MC ID / 日付 / サービス等 | JSON / HTML |
...
## Microsoft 365 Message Center 週次ダッシュボード

`.github/workflows/m365-dashboard-public.yml` は毎週月曜 07:17 JST に Microsoft Graph
から Message Center を取得し、`/m365/` に単一 HTML ダッシュボードを公開します。
Apple 製品ページを参考にした大きなタイポグラフィと余白を使いつつ、配色は本リポジトリの
Clawpilot テーマに統一しています。

公開対象はタイトル、MC ID、カテゴリ、重要度、主要日付、対象サービス、タグだけです。
`body`、`details`、テナント識別子、アクセストークンは取得結果へ保存しません。

```text
GitHub Actions
  OIDC -> Microsoft Entra ID -> Microsoft Graph
  -> scripts/Export-M365MessageCenter.ps1
  -> scripts/New-M365MessageCenterDashboard.ps1
  -> reports/m365/latest/ + GitHub Pages /m365/

GitHub Agentic Workflows
  MC本文を一時コンテキストとして読み取り
  -> 公開可能なCSA向け週次要約だけを insights.json に保存
  -> ダッシュボードを再生成
```

### 必要な Microsoft Entra ID 設定

- 既存のアプリ登録に Microsoft Graph **Application permission**
  `ServiceMessage.Read.All` を追加し、管理者同意を付与
- GitHub Actions 用 Federated Credential を構成
- Actions secrets に `AZURE_CLIENT_ID` と `AZURE_TENANT_ID` を登録
- Azure サブスクリプションは不要（`allow-no-subscriptions: true`）
...
`.github/workflows/m365-weekly-insights.md` がソースで、`gh aw compile` が生成する
`.lock.yml` もコミットします。これは Copilot Cloud Agent のタスク API とは別の仕組みで、
毎週の実行では Agentic Workflow の Copilot エンジンだけを利用します。MC本文は
`.m365-agent-context.json` としてAgent実行中だけ生成され、Artifact・Git・Pagesには保存しません。
Agentの構造化出力はcustom safe outputで検証し、`insights.json` とHTMLだけを公開します。

個人所有リポジトリでは `COPILOT_GITHUB_TOKEN`、Organization 所有リポジトリでは
`copilot-requests: write` と Organization の Copilot CLI 課金ポリシーを設定してください。
